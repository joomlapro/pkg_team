...
class TeamHelperQuery
{
	 ...
	 *
	 * @param   JRegistry  $params  An options object for the user.
	 *
	...
	public static function buildVotingQuery($params = null)
	{
		...
		if ($voting)
		{
			...
			$join   = ' LEFT JOIN #__team_rating AS v ON a.id = v.user_id';
		}
		else
		{
			$select = '';
	...
	/**
	 * Method to order the intro users array for ordering
	 * down the columns instead of across.
	 * The layout always lays the introtext users out across columns.
	 * Array is reordered so that, when users are displayed in index order
	 * across columns in the layout, the result is that the
	 * desired user ordering is achieved down the columns.
	 *
	 * @param   array    &$users   Array of intro text users.
	 ...
	 * @since   3.2
	 */
	public static function orderDownColumns(& $users, $numColumns = 1)
	{
		$count = count($users);

		// Just return the same array if there is nothing to change.
		if ($numColumns == 1 || !is_array($users) || $count <= $numColumns)
		{
			$return = $users;
		}
		// We need to re-order the intro users array.
		else
		{
			...
			// Layout the users in column order, skipping empty cells.
			$i = 0;
			...
			// Now read the $index back row by row to get users in right row/col
			...
			for ($row = 1; ($row <= $maxRows) && ($i < $count); $row++)
			{
				for ($col = 1; ($col <= $numColumns) && ($i < $count); $col++)
				{
					$return[$keys[$i]] = $users[$index[$row][$col]];
