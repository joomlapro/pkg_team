...
class Com_TeamInstallerScript
{
	...
	{
		// Adding content type for users.
		$table = JTable::getInstance('Contenttype', 'JTable');

		if (!$table->load(array('type_alias' => 'com_team.user')))
		{
			// Table column.
			$special = new stdClass;
			$special->dbtable = '#__team';
			$special->key     = 'id';
			$special->type    = 'User';
			$special->prefix  = 'TeamTable';
			...
			$content_history_options->formFile      = 'administrator/components/com_team/models/forms/user.xml';
			...
			$content_types['type_title']              = 'User';
			$content_types['type_alias']              = 'com_team.user';
			$content_types['table']                   = json_encode($table_object);
			$content_types['rules']                   = '';
			$content_types['field_mappings']          = json_encode($field_mappings);
			$content_types['router']                  = 'TeamHelperRoute::getUserRoute';
			$content_types['content_history_options'] = json_encode($content_history_options);

			$table->save($content_types);
		}

		// Adding content type for team category.
		$table = JTable::getInstance('Contenttype', 'JTable');

		if (!$table->load(array('type_alias' => 'com_team.category')))
		{
			// Table column.
			$special = new stdClass;
			...
			$content_types['type_title']              = 'User Category';
			$content_types['type_alias']              = 'com_team.category';
			$content_types['table']                   = json_encode($table_object);
			$content_types['rules']                   = '';
			$content_types['field_mappings']          = json_encode($field_mappings);
			$content_types['router']                  = 'TeamHelperRoute::getCategoryRoute';
			$content_types['content_history_options'] = json_encode($content_history_options);
		...
		if ($route != 'update')
		{
			...
			$table->load(array('name' => 'com_team'));
			$table->params = '{"user_layout":"_:default","show_title":"1","link_titles":"1","show_intro":"1","info_block_position":"0","show_category":"1","link_category":"1","show_parent_category":"1","link_parent_category":"1","show_author":"1","link_author":"0","show_create_date":"1","show_modify_date":"1","show_publish_date":"1","show_item_navigation":"1","show_vote":"0","show_readmore":"1","show_readmore_title":"1","readmore_limit":"100","show_tags":"1","show_icons":"1","show_print_icon":"1","show_email_icon":"1","show_hits":"1","show_noauth":"0","show_publishing_options":"1","show_user_options":"1","save_history":"1","history_limit":10,"float_intro":"right","float_fulltext":"right","category_layout":"_:default","show_category_heading_title_text":"1","show_category_title":"1","show_description":"1","show_description_image":"0","maxLevel":"-1","show_empty_categories":"0","show_no_users":"1","show_subcat_desc":"1","show_cat_num_users":"1","show_base_description":"1","maxLevelcat":"-1","show_empty_categories_cat":"0","show_subcat_desc_cat":"1","show_cat_num_users_cat":"1","num_leading_users":"1","num_intro_users":"4","num_columns":"2","num_links":"4","multi_column_order":"0","show_subcategory_content":"0","show_pagination_limit":"1","filter_field":"hide","show_headings":"1","list_show_date":"0","date_format":"","list_show_hits":"1","list_show_author":"1","orderby_pri":"none","orderby_sec":"rdate","order_date":"published","show_pagination":"2","show_pagination_results":"1","show_feed_link":"1","feed_summary":"0","feed_show_readmore":"0"}';
			$table->store();
		}
	}
	...
	public function install(JAdapterInstance $adapter)
	{
		...
		$adapter->getParent()->setRedirectURL('index.php?option=com_team');
	}
	...
	{
		echo '<p>' . JText::_('COM_TEAM_UNINSTALL_TEXT') . '</p>';
	}
	...
	protected function _addCategory()
	{
		...
		// Updating all team without category to have this new one.
